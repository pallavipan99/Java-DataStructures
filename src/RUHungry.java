...
public class RUHungry {
    ...
    public void menu(String inputFile) {
        ...
        for (int i = 0; i < categoryVar.length; i++) {
            ...
            for (int x = 0; x < size; x++) {
                ...
                Dish dish = new Dish(categoryVar[i], name, ids);
                if (menuVar[i] != null) {
                    MenuNode temp = new MenuNode(menuVar[i].getDish(),
                            menuVar[i].getNextMenuNode());
                    MenuNode node = new MenuNode(dish, temp);
                    menuVar[i] = node;
                } else {
    ...
    public void addStockNode(StockNode newNode) {
        ...
        // System.out.println("hash " + stockVar[newNode.getIngredient().getID() %
        // stockVarSize]);
    }
    ...
    public StockNode findStockNode(int ingredientID) {
        // WRITE YOUR CODE HERE
        StockNode stockNode = stockVar[ingredientID % stockVarSize];
    ...
    public void updateStock(String ingredientName, int ingredientID, int stockAmountToAdd) {
        ...
        if (ingredientName != null) {
            ...
            stockNode.getIngredient().setStockLevel(stockNode.getIngredient().getStockLevel() + stockAmountToAdd);
        } else if (ingredientID != -1) {
            // System.out.println(ingredientID);
            StockNode stockNode = findStockNode(ingredientID);
            stockNode.getIngredient().setStockLevel(stockNode.getIngredient().getStockLevel() + stockAmountToAdd);
        }
        // stockVarSize += stockAmountToAdd;
    }
    ...
    public void updatePriceAndProfit() {
        ...
        int ids[] = null;
        MenuNode currentCategory = null;
        for (int i = 0; i < menuVar.length; i++) {
            currentCategory = menuVar[i];
            while (currentCategory != null) {
                ids = currentCategory.getDish().getStockID();
                for (int x = 0; x < ids.length; x++) {
                    // System.out.println(ids[x]);
                    // System.out.println(findStockNode(ids[x]).getIngredient());
                    dishCost += findStockNode(ids[x]).getIngredient().getCost();
                }
                dishPrice = dishCost * 1.2;
                // System.out.println(dishPrice);
                currentCategory.getDish().setPriceOfDish(dishPrice);
                currentCategory.getDish().setProfit(dishPrice - dishCost);
    ...
    public void createStockHashTable(String inputFile) {
        ...
        while (!StdIn.isEmpty()) {
            ...
            int stockAmt = StdIn.readInt();
            Ingredient ingredient = new Ingredient(ingredientID, ingredientName, stockAmt, ingredientCost);
    ...
    public void addTransactionNode(TransactionData data) { // method adds new transactionNode to the end of LL
        ...
        TransactionNode transactionNode = transactionVar;
        if (transactionVar == null) {
            transactionVar = new TransactionNode(data, null);
            transactionNode = transactionVar;
        } else {
            while (transactionNode.getNext() != null) {
                transactionNode = transactionNode.getNext();
            }
            transactionNode.setNext(new TransactionNode(data, null));
        }
    }
    ...
    public boolean checkDishAvailability(String dishName, int numberOfDishes) {
        ...
        for (int i = 0; i < ids.length; i++) {
            // System.out.println(findStockNode(ids[i]).getIngredient().getName());
            // System.out.println(findStockNode(ids[i]).getIngredient().getStockLevel());
    ...
    public void order(String dishName, int quantity) {
        ...
        if (checkDishAvailability(dishName, quantity)) {
            while (dish != null) {
                if (dish.getDish().getName().equals(dishName)) {
                    profit = dish.getDish().getProfit();
                    break;
                }
                dish = dish.getNextMenuNode();
            }
            addTransactionNode(new TransactionData("order", dishName, quantity, profit * quantity, true));
            int ids[] = dish.getDish().getStockID();
            for (int i = 0; i < ids.length; i++) {
                // System.out.println(findStockNode(ids[i]).getIngredient().getName());
                // String name = findStockNode(ids[i]).getIngredient().getName();
                updateStock(findStockNode(ids[i]).getIngredient().getName(), ids[i], -quantity);
                // System.out.println(findStockNode(ids[i]).getIngredient().getStockLevel());

                // System.out.println("True " +
                // findStockNode(ids[i]).getIngredient().getName());

            }
        } else {

            dish = findDish(dishName);
            addTransactionNode(new TransactionData("order", dishName, quantity, 0,
                    false));
            String category = dish.getDish().getCategory();
            MenuNode dishes1 = menuVar[0];
            MenuNode dishes2 = menuVar[0];
            for (int x = 0; x < categoryVar.length; x++) {
                if (categoryVar[x].equals(category)) {
                    dishes1 = menuVar[x];
                    dishes2 = menuVar[x];
                }
            }
            while (dishes1 != null) {
                // System.out.println(dishes1.getDish().getName());
                if (dishes1.getDish().getName().equals(dishName)) {
                    afterItem = true;
                    break;
                }
                dishes1 = dishes1.getNextMenuNode();

            }
            dishes1 = dishes1.getNextMenuNode();
            while (dishes1 != null) {
                // System.out.println(dishes1.getDish().getName());
                if (checkDishAvailability(dishes1.getDish().getName(), quantity)) {
                    int ids[] = dishes1.getDish().getStockID();
                    found = true;
                    addTransactionNode(new TransactionData("order", dishes1.getDish().getName(), quantity,
                            dishes1.getDish().getProfit() * quantity, true));
                    for (int i = 0; i < ids.length; i++) {
                        // System.out.println(findStockNode(ids[i]).getIngredient().getName());
                        updateStock(findStockNode(ids[i]).getIngredient().getName(), ids[i], -quantity);
                    ...
                    // updateStock(findStockNode(dishes)).getIngredient().getName(), ids[i],
                    // -quantity);
                    break;
                } else if (!dishes1.getDish().getName().equals(dishName)) {
                    // System.out.println("dish " + dishes1.getDish().getName());
            ...
            // System.out.println(dishes2.getDish().getName());
            // System.out.println("Dish name " + dishName);
            if (found == false && !(dishes2.getDish().getName().equals(dishName))) {

                // dishes = menuVar[x];
                // System.out.println("else" + dishes2.getDish().getName());

                while (dishes2 != null && !dishes2.getDish().getName().equals(dishName)) {

                    // System.out.println("dishes" + dishes2.getDish().getName());
                    if (checkDishAvailability(dishes2.getDish().getName(), quantity)) {
                        int ids[] = dishes2.getDish().getStockID();
                        addTransactionNode(new TransactionData("order", dishes2.getDish().getName(), quantity,
                                dishes2.getDish().getProfit() * quantity, true));
                        for (int i = 0; i < ids.length; i++) {
                            // System.out.println(findStockNode(ids[i]).getIngredient().getName());
                            updateStock(findStockNode(ids[i]).getIngredient().getName(), ids[i], -quantity);
                        ...
                        break;
                    } else if (!dishes2.getDish().getName().equals(dishName)) {
                        // System.out.println(dishes2.getDish().getName());
                        addTransactionNode(new TransactionData("order", dishes2.getDish().getName(), quantity,
                                0, false));

                    }
                    dishes2 = dishes2.getNextMenuNode();
    ...
    public double profit() {
        ...
        while (transactionNode != null) {
            profit += transactionNode.getData().getProfit();
            transactionNode = transactionNode.getNext();
        }
    ...
    public void donation(String ingredientName, int quantity) {
        ...
        if (profit() > 50 && findStockNode(ingredientName).getIngredient().getStockLevel() >= quantity) {
            ...
            updateStock(ingredientName, findStockNode(ingredientName).getIngredient().getID(), 0);
        }
    }
    ...
    public void restock(String ingredientName, int quantity) {
        ...
        if (updatedStock) {
            TransactionData data = new TransactionData("restock", ingredientName, quantity,
                    -findStockNode(ingredientName).getIngredient().getCost() * quantity, true);
